...
- (void) viewDidLoad {
	...
	self.navigationItem.rightBarButtonItems = @[
												[UIBarButtonItem SA_itemWithSystemItem: UIBarButtonSystemItemRefresh target: self action: @selector(sync:)],
												[UIBarButtonItem SA_itemWithTitle: @"All" target: self action: @selector(syncAll:)]
												];
	[super viewDidLoad];
}
...
- (void) setEntityName:(NSString *)entityName {
	...
	self.title = $S(@"%@ (%d)", self.entityName, (UInt16) self.recordsTableView.records.count);
}
...
- (IBAction) sync: (id) sender {
	...
	[[MM_SyncManager sharedManager] fetchRequiredMetaData: NO withCompletionBlock: ^{
		[[MM_SyncManager sharedManager] synchronize: @[ [MM_SFObjectDefinition objectNamed: self.entityName inContext: [MM_ContextManager sharedManager].mainMetaContext] ] withCompletionBlock: nil];
	}];
}
...
- (void) objectsImported: (NSNotification *) note {
	...
	self.title = $S(@"Imported %@ (%d)", name, (UInt16) [note.object count]);
}
