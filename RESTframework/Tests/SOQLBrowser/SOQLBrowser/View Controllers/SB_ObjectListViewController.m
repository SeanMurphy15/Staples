...
- (UITableViewCell *) tableView: (UITableView *) tableView cellForRowAtIndexPath: (NSIndexPath *) indexPath {
	...
	if ([[MM_OrgMetaData sharedMetaData] isObjectSynced: name]) {
		...
		int			count = [[MM_ContextManager sharedManager].mainContentContext numberOfObjectsOfType: name matchingPredicate: nil];
		if (count) cell.textLabel.text = $S(@"%@ (%d)", name, count);
	} else {
