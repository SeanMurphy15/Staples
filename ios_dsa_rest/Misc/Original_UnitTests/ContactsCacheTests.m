...
- (void) testNoContacts
{
    ...
    [self.cache contactsCacheLoader:(ContactsCacheLoader*)loaderMock contactCount:0];
    [self.cache contactsCacheLoaderLoadComplete:loaderMock];
    
    NSUInteger count = self.cache.contactCount;
    
    STAssertTrue((count == 0), @"contact count not zero");
    STAssertEqualObjects([self.cache contactAtIndex:10], nil, @"out of range contact request did not return nil");
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(noContactsSearchComplete:) name:ContactsCacheNotification_SearchComplete object:self.cache];
    [[NSNotificationCenter defaultCenter] addObserverForName:ContactsCacheNotification_SearchComplete
                                                  ...
                                                  usingBlock:^(NSNotification *note) {
                                                      ...
                                                      STAssertTrue((count == 0), @"search count not zero");
                                                      STAssertEqualObjects([self.cache contactAtIndex:10], nil, @"out of range search request did not return nil");
    ...
    STAssertFalse(timedOut, @"search took longer than timeout");
}
...
- (void) testOneContact
{
    ...
    //now we can fake the load with the delegate methods
    [self.cache contactsCacheLoader:(ContactsCacheLoader*)loaderMock contactCount:1];
    ...
    [self.cache contactsCacheLoader:loaderMock contactsLoaded:[NSArray arrayWithObject:dict]];
    [self.cache contactsCacheLoaderLoadComplete:loaderMock];
    
    NSUInteger count = self.cache.contactCount;
    
    STAssertTrue((count == 1), @"contact count not one");
    
    NSDictionary* contact = [self.cache contactAtIndex:0];
    STAssertNotNil(contact, @"contact was nil");
    
    STAssertEqualObjects(dict, contact, @"contact did not equal original");
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:ContactsCacheNotification_SearchComplete
                                                  ...
                                                  usingBlock:^(NSNotification *note) {
                                                      ...
                                                      STAssertTrue((count == 1), @"search count wrong");
                                                      STAssertEqualObjects([self.cache contactAtIndex:10], nil, @"out of range search request did not return nil");
...
- (void) testLotsOfContacts
{
    ...
    //NSString* path = [[NSBundle mainBundle] pathForResource:@"contacts_100000" ofType:@"json"];
    NSString *path = [[NSBundle bundleForClass:[ContactsCacheTests class]] pathForResource:@"contacts_100000" ofType:@"json"];
    NSString* contactsJSON = [NSString stringWithContentsOfFile: path
                                                       encoding:NSUTF8StringEncoding
                                                          error:&error];
    NSDictionary* parsedContacts = [contactsJSON objectFromJSONString];
    NSArray* allContacts = [parsedContacts objectForKey:@"result"];
    NSUInteger contactsWithEmailCount = [[parsedContacts objectForKey:@"withEmail"] integerValue];
    NSUInteger numberOfBrookes = [[parsedContacts objectForKey:@"numberOfBrookesWithEmail"] integerValue];
    ...
    [self.cache contactsCacheLoader:loaderMock contactsLoaded:allContacts];
    [self.cache contactsCacheLoaderLoadComplete:loaderMock];
    
    NSUInteger count = self.cache.contactCount;
    
    STAssertTrue((count == contactsWithEmailCount), @"contact count wrong");
...
//    STAssertEqualObjects(dict, contact, @"contact did not equal original");
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:ContactsCacheNotification_SearchComplete
                                                  ...
                                                  usingBlock:^(NSNotification *note) {
                                                      ...
                                                      STAssertTrue((count == numberOfBrookes), @"search count not correct");
