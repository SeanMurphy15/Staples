...
- (void) getMediaModes:(CDVInvokedUrlCommand*)command
{
   // NSString* callbackId = [arguments objectAtIndex:0];
	...
	NSError				*error;
    NSString* json = [[NSJSONSerialization dataWithJSONObject: modes options: 0 error: &error] stringValue];
    NSString* jsString = [NSString stringWithFormat:@"navigator.device.capture.setSupportedModes(%@);", json];
