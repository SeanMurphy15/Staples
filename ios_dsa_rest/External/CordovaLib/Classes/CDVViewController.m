...
- (int) executeCommandsFromJson:(NSString*)queuedCommandsJSON
{
	...
	NSError				*error = nil;
	NSData				*data = [NSData dataWithBytes: queuedCommandsJSON.UTF8String length: strlen(queuedCommandsJSON.UTF8String)];

	NSArray* queuedCommands = [NSJSONSerialization JSONObjectWithData: data options: NSJSONReadingMutableLeaves error: &error];
...
#ifdef DEBUG
			NSError				*error;
			NSString* commandJson = [[NSJSONSerialization dataWithJSONObject: jsonEntry options: 0 error: &error] stringValue];
