...
AC.VR.options = {
	imageIndexOffset: 0,        // (number)   maps the position [0,0] to image 001.jpg, etc.
	...
	grabRotateDistance: 700,   // (number)   pixels the cursor must travel to view a full axis
	...
	introSpins: 0,            // (number)   times the VR rotates 360 degrees in the intro
	introDuration: 10,           // (number)   intro duration in seconds
...
Object.extend(AC.VR.prototype, {
	...
	initialize: function(container, imagePath, totalFrames, options){
		...
		//var this.customGrabHistory;
		//this.customGrabHistory.Y = null;
		// options
	...
	makeInteractive: function(){
		if (this.options.grabbable) {
			...
			this.bindTouchEnd = this.customTouchEnd.bind(this);
			...
			this.vr.observe(this.getStr('mouseup'), this.bindTouchEnd);
	...
	customTouchEnd: function(event) {
		if (event.changedTouches && event.changedTouches.length > 0) {
			//alert(event.changedTouches[0].pageX);
			//var sampleDiv = document.getElementById('jsSampleDiv');
			//sampleDiv.innerHTML = event.changedTouches[0].screenX - document.getElementById('main').offsetLeft + '<br/>' + this.currentPos;
			//sampleDiv.innerHTML = event.changedTouches[0].pageY - document.getElementById('main').offsetTop + '<br/>' + this.currentPos;
			//sampleDiv.innerHTML = document.getElementById('main').offsetLeft;
			
			//sampleDiv.innerHTML = document.getElementById('viewer').childNodes[0].childNodes[0].height;
			
			//var pos_x = event.pageX-document.getElementById("pointer_div").offsetLeft;
			//var pos_y = event.pageY-document.getElementById("pointer_div").offsetTop;
			
			var tempA = imagePositions[this.currentPos[0]];
			
			//alert(tempA);
			
			var found = false;

			//alert("X: " + (event.changedTouches[0].pageX - document.getElementById('main').offsetLeft) + " Y: " + (event.changedTouches[0].pageY - document.getElementById('main').offsetTop));
			
			if (tempA) {
				for (var i=0; i<tempA.length; i++) {
					found = false;
					if ((event.changedTouches[0].pageX - document.getElementById('main').offsetLeft) >= tempA[i][0]) {
						if ((event.changedTouches[0].pageX - document.getElementById('main').offsetLeft) <= tempA[i][1]) {
							if ((event.changedTouches[0].pageY - document.getElementById('main').offsetTop) >= tempA[i][2]) {
								if ((event.changedTouches[0].pageY - document.getElementById('main').offsetTop) <= tempA[i][3]) {
									


									//sampleDiv.innerHTML = tempA[i][4];
									found = true;
									//alert('found');
								} else {
									//sampleDiv.innerHTML = '<br/>';
					...
					if (found) {
						...
						for (var z=0; z<allDivs.length; z++) {
							var elToHide = document.getElementById(allDivs[z]);
						...
						var el = document.getElementById(tempA[i][4]);
            			el.style.opacity = "1";
			...
			//if (this.currentPos[0] == 0) {
				//sampleDiv.innerHTML = event.changedTouches[0].pageX - document.getElementById('main').offsetLeft + '<br/>' + this.currentPos;
				
			//	if ((event.changedTouches[0].pageX - document.getElementById('main').offsetLeft) >= 395 && (event.changedTouches[0].pageX - document.getElementById('main').offsetLeft) <= 450) {
			//		if ((event.changedTouches[0].pageY - document.getElementById('main').offsetTop) >= 280 && (event.changedTouches[0].pageY - document.getElementById('main').offsetTop) <= 350) {
			//			sampleDiv.innerHTML = 'touched on front right wheel';
			//		}
			//	}
			//}
		}
	...
	onGrabStart: function(event){
		...
		if (event.touches && event.touches.length > 0) {
			customGrabHistory.startX = event.touches[0].screenX;
			customGrabHistory.startY = event.touches[0].screenY;
		
		}

		
		
		//alert(customGrabHistory.X);

		//alert(event.touches[0].screenX);
		//alert(customGrabHistory.X);
		//alert(this.customGrabHistory);
		//this.customGrabHistory.X = event.touches[0].screenX;
		//customGrabHistory.Y = event.touches[0].screenY;
